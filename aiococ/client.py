...
class Client:
    ...
    async def search_clans(self, *, name: str=None, war_frequency: str=None,
        ...
        All parameters are optional, however one must be passed
    ...
    async def get_clan(self, tag, cache=False, fetch=True, json=False):
        """
        Search for a clan with a given tag.
    ...
    async def get_members(self, clan_tag, cache=False, fetch=True):
        """
        Get clan members for a given clan tag.
    ...
    async def get_warlog(self, clan_tag, cache=False, fetch=True, json=False):
        """
        Get the war log of a clan with the given tag
    ...
    async def get_current_war(self, clan_tag, cache=False, fetch=True, json=False):
        """
        Get the current war of a clan with the given tag
    ...
    async def get_league_group(self, clan_tag, cache=False, fetch=True, json=False):
        """
        Get the league group of a clan with given tag
    ...
    async def get_league_war(self, war_tag, cache=False, fetch=True, json=False):
        """
        Search for a current league war with a given war tag.
    ...
    async def get_location(self, location_id, cache=False, fetch=True, json=False):
        """
        Get location information for a location id
    ...
    async def get_league(self, league_id, cache=False, fetch=True, json=False):
        """
        Get a league with a given league ID.
    ...
    async def get_seasons(self, league_id):
        """
        Search for seasons with a given league ID.
    ...
    async def get_season_info(self, league_id, season_id, cache=False, fetch=True, json=False):
        """
        Get list of basic player info in given league and season IDs.
        ...
        :return: :class:`list` of :class:`LeaguePlayer` if ``json=False``, else :class:`dict`
        """
        if cache:
            try:
                data = self._seasons[league_id][season_id]
                return check_json(LeaguePlayer, data, json)
        ...
        players = list(LeaguePlayer(data=n) for n in r.get('items', []))
    ...
    async def get_player(self, player_tag, cache=False, fetch=True, json=False):
        """
        Get complete player info for a given tag.
