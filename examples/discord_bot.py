...
async def clan_info(ctx, clan_tag):
    ...
    for a in clan.members:
        if a.role == a.role.leader:
            leader = a
    ...
    e.add_field(name="Leader", value=f'{leader}', inline=False)
...
async def clan_member(ctx, clan_tag):
    ...
    for i, a in enumerate(clan.members, start=1):
        member += f'``{i}.`` {a.name}\n'
    embed = discord.Embed(colour=discord.Colour.red(), title= f'Members of {clan.name}',description= f'{member}')
