...
client = discordlinks.login(
    os.environ.get("LINKS_API_USERNAME"), os.environ.get("LINKS_API_PASSWORD")
)
...
async def main():
    ...
    print(f"Link for player tag {player_tag} has been updated to have discord id {new_discord_id}")

    # delete a link
    await client.delete_link(player_tag)
    print(f"Link for player tag {player_tag} has been removed from the database.")
